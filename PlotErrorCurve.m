function emax = PlotErrorCurve(A,B,C,D,G,select,x0,Q,R,Pfalse,ConLvl,policy,probIncrease,precision)
...
if ~select % If time series of max(e) were requested
    locPolicy = FormGlobalPeriodicPolicy(q,min(maxSearch,policy(1)),policy(2));
    ...
    for i = 2:policy
        ...
        locPolicy = FormGlobalPeriodicPolicy(q,duration,i); %keep duration constant to keep alpha stable
