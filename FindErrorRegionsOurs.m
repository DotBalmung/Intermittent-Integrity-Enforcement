function [Regions,ErrorNorms] = FindErrorRegionsOurs(A,B,C,D,G,x0,Q,R,h,ConLvl,policy,probIncrease,precision)
    ...
    %       up to 10^(-precision).
    %Outputs:
    %   Regions: Cell structure. Each cell contains points from the region
...
for k=1:steps
    Regions{k} = inv(RegLimits{k});
    %ErrorNorms(k)=max(sqrt(sum(Regions{k}.^2)));
